"""
KOKAHOUSE — AI Telegram Bot
Бот для общения с клиентами. Claude AI + amoCRM.
...
CHANNEL_ID      = os.getenv("CHANNEL_ID", "@kokahouse")  # канал для постинга
...
DIRECTOR_SYSTEM = """Ты — персональный AI-директор компании KOKAHOUSE.
...
[Чёткое описание что нужно реализовать, какие данные получать, откуда (amoCRM/Telegram), как отображать результат. Максимально конкретно, как техническое задание.]
---
...
def director_find_client(query: str) -> list:
    """Найти клиента в amoCRM по имени или Telegram ID."""
    import urllib.parse
    amo_map = _load_amo_map()                      # tg_id -> {contact_id, lead_id}
    contact_to_tg = {v["contact_id"]: tg for tg, v in amo_map.items()}

    found = []  # список (contact_dict, tg_id)

    # Поиск по Telegram ID, если запрос — число
    if query.lstrip("-").isdigit():
        tg_id = int(query)
        if tg_id in amo_map:
            cid = amo_map[tg_id]["contact_id"]
            r = amo_request("GET", f"contacts/{cid}")
            if isinstance(r, dict) and r.get("id"):
                found.append((r, tg_id))

    # Поиск по имени через amoCRM
    if not found:
        q = urllib.parse.quote(query)
        r = amo_request("GET", f"contacts?query={q}&limit=5")
        emb = r.get("_embedded", {}) if isinstance(r, dict) else {}
        for c in (emb.get("contacts", []) if isinstance(emb, dict) else []):
            found.append((c, contact_to_tg.get(c.get("id"))))

    clients = []
    for c, tg_id in found[:5]:
        lead_id = amo_map.get(tg_id, {}).get("lead_id") if tg_id else None
        clients.append({
            "name": c.get("name", "—"),
            "contact_id": c.get("id"),
            "tg_id": tg_id,
...
def sync_to_amo(tg_id: int, name: str, username: str,
    ...
    Все клиенты → контакт в Списках + история переписки в заметках контакта.
    Сделки создаются вручную из Списков.
    """
    if not AMO_TOKEN:
        return
    try:
        ...
        # ── Заметка с перепиской → в контакт ─────────────────────────────
...
def director_get_revenue_stats(days: int, group_by: str = "итого") -> dict:
    """Статистика выручки. Источник: amoCRM."""
    ...
    # amoCRM токен не задан — статистика недоступна
    return {
        "source": "amoCRM не подключён",
        "error": "AMO_LONG_TOKEN не задан — статистика выручки недоступна",
        "period_days": days,
        "current": {},
        "previous": {},
        "delta": None,
...
def director_list_leads(qualification: str, limit: int = 10) -> list:
    ...
    # amoCRM токен не задан
    return [{"error": "amoCRM не подключён (AMO_LONG_TOKEN не задан)"}]
            ...
            for block in response.content:
                ...
                try:
                    if tool == "get_stats":
                        ...
                        # Ищем клиента в amoCRM по имени
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_owner(user):
        ...
        if intent == "post":
            ...
            await context.bot.send_chat_action(chat_id=update.effective_chat.id, action="typing")
            prompt = f"""Напиши продающий пост для Telegram-канала KOKAHOUSE.
Тема: {text}
Требования: 3-4 абзаца, живой стиль, цены если знаешь, в конце призыв писать @kokahouse_Yulia.
    ...
    if user.id in pending_kp and text.strip().lower() in [
        ...
        try:
            ...
            await context.bot.send_document(
                ...
                caption=f"Коммерческое предложение KOKAHOUSE\n{kp_data['product']}\nИтого: {kp_data['total']:,} ₽".replace(',', ' ')
            )
...
async def _send_and_update(update, context, user, result, original_text):
    """Отправить ответ клиенту и синхронизировать с amoCRM."""
...
async def cmd_ai_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = f"""Напиши продающий пост для Telegram-канала мебельной компании KOKAHOUSE.
...
— В конце: призыв написать в личку боту @kokahouse_Yulia
...
async def cmd_kp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 3:
        await update.message.reply_text(
            ...
            "или найди в amoCRM по карточке контакта",
            parse_mode="Markdown"
        )
        return
...
async def daily_report(bot):
    ...
    try:
        ...
        msg = (
            f"☀️ *Доброе утро! Отчёт KOKAHOUSE*\n\n"
...
AUTO_POST_TOPICS = [
    ...
    "Как сделать заказ в KOKAHOUSE — от запроса до доставки. Пошаговый процесс",
...
— В конце: "Подробнее и расчёт доставки — @kokahouse_Yulia"
— 2-3 emoji уместно
— Форматирование Markdown (жирный, курсив)
— Без хэштегов
...
def main():
    ...
    logger.info("🤖 KOKAHOUSE Bot запущен")
